...
// src/http/controllers/question/delete.spec.ts
...
(0, import_vitest.describe)("Delete Question (E2E)", () => {
  (0, import_vitest.it)("Should be delete a question by id", async () => {
    const { token } = await createAndAuthenticateTest(app, {});
    const questionCreated = await (0, import_supertest2.default)(app.server).post("/question").set("Authorization", `Bearer ${token}`).send({
      title: "Test Question 1",
      content: "content",
      anonymous: false
    });
    const question = questionCreated.body;
    await (0, import_supertest2.default)(app.server).delete("/question").set("Authorization", `Bearer ${token}`).send({
      question_id: question.id
    });
  });
  (0, import_vitest.it)("User cannot be delete a question without a session", async () => {
    ...
    const question = questionCreated.body;
    const response = await (0, import_supertest2.default)(app.server).delete("/question").set("Authorization", `Bearer ${token}`).send({
      question_id: question.id
    });
  ...
  (0, import_vitest.it)("User should be delete only own question", async () => {
    const { token } = await createAndAuthenticateTest(app, {
    ...
    const question = questionCreated.body;
    const response = await (0, import_supertest2.default)(app.server).delete("/question").set("Authorization", `Bearer ${invalidToken}`).send({
      question_id: question.id
    });
