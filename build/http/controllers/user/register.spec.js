...
(0, import_vitest.describe)("Register user (E2E))", () => {
  (0, import_vitest.it)("Should be register a new user", async () => {
  ...
  (0, import_vitest.it)("Should not be able to register a new user with an email that is already registered", async () => {
    const user = {
      name: "John Doe",
      username: "johndoe",
      email: "johndoe@does.com",
    ...
    await (0, import_supertest.default)(app.server).post("/users").send(user);
    const response = await (0, import_supertest.default)(app.server).post("/users").send(user);
    (0, import_vitest.expect)(response.statusCode).toBe(409);
