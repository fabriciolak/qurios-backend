...
// src/http/controllers/user/update.spec.ts
...
async function register(request2, reply) {
  ...
  const { name, username, email, password } = userBodySchema.parse(request2.body);
  try {
...
async function authenticate(request2, reply) {
  ...
  const { email, password } = authenticateBodySchema.parse(request2.body);
  try {
...
async function update(request2, reply) {
  ...
  const { email, name, username } = updateBodySchema.parse(request2.body);
  try {
    const updateUseCase = await fetchUpdateUserUseCase();
    await updateUseCase.execute(request2.user.sub, {
      email,
      name,
      username
    });
...
async function userProfile(request2, reply) {
  try {
    const userProfileParamsSchema = import_zod4.z.object({
      userId: import_zod4.z.string().uuid()
    });
    const { userId } = userProfileParamsSchema.parse(request2.params);
    const usersRepository = new PrismaUsersRepository();
...
async function update2(request2, reply) {
  ...
  try {
    ...
    const { questionId } = updateQuestionParamsSchema.parse(request2.params);
...
async function deleteQuestion(request2, reply) {
  const deleteQuestionBodySchema = import_zod7.z.object({
    question_id: import_zod7.z.string().uuid()
  });
  try {
    const { question_id } = deleteQuestionBodySchema.parse(request2.body);
    const userHavePermission = await prisma.question.findFirst({
      where: {
        id: question_id,
        user_id: request2.user.sub
...
async function questionList(request2, reply) {
...
app.get("/", (request2, reply) => {
  console.log(request2.method, request2.url);
...
(0, import_vitest.describe)("Update user (E2E))", () => {
  (0, import_vitest.it)("Should be update a user", async () => {
    ...
    const sessionResponse = await (0, import_supertest.default)(app.server).post("/sessions").send({ email: "johndoe@does.com", password: "123456" }).expect(200);
    const { token } = sessionResponse.body;
    await (0, import_supertest.default)(app.server).put("/users").set("Authorization", `Bearer ${token}`).send({ email: "newjohndoe@does.com" }).expect(200);
  });
  (0, import_vitest.it)("Should not be update a user with same email", async () => {
    await (0, import_supertest.default)(app.server).post("/users").send({
      name: "John Doe",
      username: "johndoe2",
    ...
    const sessionResponse = await (0, import_supertest.default)(app.server).post("/sessions").send({ email: "johndoe2@does.com", password: "123456" }).expect(200);
    const { token } = sessionResponse.body;
    await (0, import_supertest.default)(app.server).put("/users").set("Authorization", `Bearer ${token}`).send({ email: "johndoe2@does.com" }).expect(409);
  });
});
