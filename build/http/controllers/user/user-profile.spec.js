...
// src/http/controllers/user/user-profile.spec.ts
...
(0, import_vitest.describe)("Profile user (E2E))", () => {
  (0, import_vitest.it)("Should be get user profile", async () => {
    const { token, user } = await createAndAuthenticateTest(app, {});
    const response = await (0, import_supertest2.default)(app.server).get(`/users/profile/${user.id}`).set("Authorization", `Bearer ${token}`).send();
    (0, import_vitest.expect)(response.body).toEqual(
      import_vitest.expect.objectContaining({
        id: import_vitest.expect.stringMatching(user.id)
      })
    );
  });
});
